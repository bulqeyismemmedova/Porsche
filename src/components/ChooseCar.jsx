...
const ChooseCar = ({ data, sidebar, setSideBar, firstCar, setFirstCar, secondCar, setSecondCar }) => {
    ...
    return (
        <>
            <div onClick={() => setSideBar(false)} className={`fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity duration-300 z-40 ${sidebar ? "block opacity-100" : "hidden opacity-0"}`} />
            <div className={`fixed top-0 right-0 h-screen w-full md:w-[90%] bg-white z-50 shadow-2xl flex flex-col transition-transform duration-700 ease-in-out transform ${sidebar ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <div className="flex flex-col lg:flex-row gap-10 p-6 overflow-y-auto mt-3">
                    <div className="block max-w-xs w-full overflow-y-auto">
                                ...
                                <div key={variant} className="mb-8">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {variantCars.map((car, i) => (
                                            <div onClick={() => handleCarSelect(car)}  key={i}  
                                                className={`bg-gray-100 rounded-xl p-4 md:p-5  cursor-pointer transition-all hover:shadow-lg ${
                                                ...
                                                <p className="text-xs text-gray-500 mt-2">PDK {car.transmission.join("/")} , {car.drive}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                ...
                <div className='bg-white startShadow'>
                    <button  onClick={handleCompare}
                        className={`text-sm m-6 rounded py-3 px-6 transition ${firstCar && secondCar
